...
if (class_exists('AmazonWooCommerceServerStatusAjax') != true) {
    class AmazonWooCommerceServerStatusAjax extends AmazonWooCommerceServerStatus
    {
        ...
        */
        public function __construct( $the_plugin=array() )
        {
			...
			// ajax  helper
			add_action('wp_ajax_AmazonWooCommerceServerStatusRequest', array( $this, 'ajax_request' ));
		}
		...
		*/
		public function ajax_request()
		{
			...
			if( in_array( 'check_memory_limit', array_values($actions)) ){
				...
				$html = array();
            	if ( $memory < 127108864 ) {
            		$html[] = '<div class="AmazonWooCommerce-message AmazonWooCommerce-error">' . sprintf( __( '%s - We recommend setting memory to at least 128MB. See: <a href="%s">Increasing memory allocated to PHP</a>', $this->the_plugin->localizationName ), size_format( $memory ), 'http://codex.wordpress.org/Editing_wp-config.php#Increasing_memory_allocated_to_PHP' ) . '</div>';
            	} else {
            		$html[] = '<div class="AmazonWooCommerce-message AmazonWooCommerce-success">' . size_format( $memory ) . '</div>';
            	}
			...
			if( in_array( 'export_log', array_values($actions)) ){
				...
				header( 'Content-Disposition: attachment; filename="AmazonWooCommerce-logs.html"' );
			...
			if( in_array( 'remote_get', array_values($actions)) ){
				...
				$return = array(
					...
					'html' 		=> ( $status == true ? '<div class="AmazonWooCommerce-message AmazonWooCommerce-success">' : '<div class="AmazonWooCommerce-message AmazonWooCommerce-error">' ) . $msg . '</div>' 
				);
        	}
