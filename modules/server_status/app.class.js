...
AmazonWooCommercePriceUpdateMonitor = (function ($) {
    "use strict";
	...
	function syncProduct( row, id )
	{
		...
		jQuery.post(ajaxurl, {
			'action' 		: 'AmazonWooCommerceSyncProduct',
			'id'			: id,
			...
			if( response.status == 'valid' ){
				...
				row.find(".AmazonWooCommerce-data-price").html( html_price );
				
				row.find(".AmazonWooCommerce-data-last_sync_date").html( response.data.last_sync_date );
			}
	...
	function stress_test_step1(that)
	{
		...
		// get the ASIN code 
		var ASIN = $("#AmazonWooCommerce-test-ASIN").val();
		
		// show the timeline 
		var timeline = $(".AmazonWooCommerce-test-timeline");
		timeline.show();
		
		// show log container
		var logs_container = $(".AmazonWooCommerce-table.AmazonWooCommerce-logs");
		logs_container.show();
		...
		var status_box_step = $("#stepid-step1").find(".AmazonWooCommerce-step-status"); 
		
		jQuery.post(ajaxurl, {
			'action' 		: 'AmazonWooCommerceStressTest',
			'ASIN'			: ASIN,
			...
			logs_container.find("#logbox-step1 .AmazonWooCommerce-log-details").val( JSON.stringify( response.log ) );
			
			// show the status into timeline
			status_box_step.removeClass('AmazonWooCommerce-loading-inprogress');
				
			if( response.status == 'valid' ){
				status_box_step.addClass('AmazonWooCommerce-loading-success');
				...
				status_box_step.addClass('AmazonWooCommerce-loading-error');
		...
		.error(function(response) { 
			...
			logs_container.find("#logbox-step2 .AmazonWooCommerce-log-details").val( messge );
			
			status_box_step.removeClass('AmazonWooCommerce-loading-inprogress').addClass('AmazonWooCommerce-loading-error');
			status_box_step.html("error");
		});
	}
	...
	function stress_test_step3( timeline, logs_container )
	{
		...
		var status_box_step = $("#stepid-step3").find(".AmazonWooCommerce-step-status");
		status_box_step.addClass('AmazonWooCommerce-loading-inprogress');
		...
		jQuery.post(ajaxurl, {
			...
			logs_container.find("#logbox-step3 .AmazonWooCommerce-log-details").val( JSON.stringify( response.log ) );
		...
		.error(function(response) { 
			...
			var messge = "Please contact your server administrator and ask about this error: " + response.status + ": " + response.statusText;
			logs_container.find("#logbox-step3 .AmazonWooCommerce-log-details").val( messge );
	...
	function stress_test_step2( timeline, logs_container )
	{
		...
		var status_box_step = $("#stepid-step2").find(".AmazonWooCommerce-step-status");
		status_box_step.addClass('AmazonWooCommerce-loading-inprogress');
		...
		jQuery.post(ajaxurl, {
			...
			// save the log
			logs_container.find("#logbox-step2").css('opacity', 1);
			logs_container.find("#logbox-step2 .AmazonWooCommerce-log-details").val( JSON.stringify( response.log ) );
		...
		.error(function(response) { 
			...
			var messge = "Please contact your server administrator and ask about this error: " + response.status + ": " + response.statusText;
			logs_container.find("#logbox-step1 .AmazonWooCommerce-log-details").val( messge );
	...
	{
		jQuery.post(ajaxurl, {
			'action' 		: 'AmazonWooCommerceServerStatusRequest',
			'sub_action'	: action,
			'debug_level'	: debug_level
		}, function(response) {
	...
	function triggers()
	{
		maincontainer.on('click', 'input.AmazonWooCommerce-sync_now', function(e){
			e.preventDefault();
		...
		maincontainer.on('click', 'a.AmazonWooCommerceStressTest', function(e){
			e.preventDefault();
		...
		maincontainer.on('click', '.AmazonWooCommerce-log-title a', function(e){
			e.preventDefault();
			
			var that = $(this),
				parent = that.parent('div');
			
			parent.next(".AmazonWooCommerce-log-details").show();
		});
		
		maincontainer.on('click', '.AmazonWooCommerce-export-logs', function(e){
			e.preventDefault();
			
			export_logs( $(".AmazonWooCommerce-panel-content > .AmazonWooCommerce-table" ) );
		});
		
		$(".AmazonWooCommerce-loading-ajax-details").each( function(){
			var that = $(this),
