	    ...
	    */
		...
		{ 
			...
			if( isset($id) && trim($id) != "" && !isset($this->boxes[$id]) ){
				...
				$box[] = '<div class="AmazonWooCommerce-dashboard-status-box">';
		...
		public function formatRow( $content=array() )
		{
			...
			$html[] = '<div class="AmazonWooCommerce-dashboard-status-box-row">';
			...
			if( isset($content['ajax_content']) && $content['ajax_content'] == true ){
				$html[] = '<div class="AmazonWooCommerce-dashboard-status-box-content is_ajax_content">';
		...
		public function aateam_products()
		{
			...
			$html[] = '<ul class="AmazonWooCommerce-aa-products-tabs">';
			$html[] = 	'<li class="on">';
			$html[] = 		'<a href="javascript: void(0)" class="AmazonWooCommerce-aa-items-codecanyon">CodeCanyon</a>';
			$html[] = 	'</li>';
			$html[] = 	'<li>';
			$html[] = 		'<a href="javascript: void(0)" class="AmazonWooCommerce-aa-items-themeforest">ThemeForest</a>';
			$html[] = 	'</li>';
			$html[] = 	'<li>';
			$html[] = 		'<a href="javascript: void(0)" class="AmazonWooCommerce-aa-items-graphicriver">GraphicRiver</a>';
			$html[] = 	'</li>';
			$html[] = '</ul>';
		...
		{
			$html = array();
			$html[] = '<div class="AmazonWooCommerce-audience-graph" id="AmazonWooCommerce-audience-visits-graph" data-fromdate="' . ( date('Y-m-d', strtotime("-1 week")) ) . '" data-todate="' . ( date('Y-m-d') ) . '"></div>';
		...
		{
			$html = array();
			$html[] = '<div class="AmazonWooCommerce-website-preview">';
			...
			$html[] = 	'<h4>Thank you for buying: <a target="_blank" href="http://codecanyon.net/item/plugin/' . ( get_option('AmazonWooCommerce_register_item_id') ) . '">' . ( get_option('AmazonWooCommerce_register_item_name') ) . '</a></h4>';
			$html[] = 	'<h4>Licence: <a target="_blank" href="http://codecanyon.net/licenses/regular_extended">' . ( get_option('AmazonWooCommerce_register_licence') ) . '</a></h4>';
			$html[] = '</div>';
...
//$AmazonWooCommerceDashboard = AmazonWooCommerceDashboard::getInstance( isset($module) ? $module : array() );
//$AmazonWooCommerceDashboard = new AmazonWooCommerceDashboard( isset($module) ? $module : array() );
// $AmazonWooCommerce->cfg, ( isset($module) ? $module : array()) 
