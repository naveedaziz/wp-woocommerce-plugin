...
if (class_exists('AmazonWooCommerceDashboardAjax') != true) {
    class AmazonWooCommerceDashboardAjax extends AmazonWooCommerceDashboard
    {
        ...
        */
        public function __construct( $the_plugin=array() )
        {
			...
			// ajax  helper
			add_action('wp_ajax_AmazonWooCommerceDashboardRequest', array( &$this, 'ajax_request' ));
		}
		...
		*/
		public function ajax_request()
		{
			...
			if( in_array( 'products_performances', $actions) ){
				...
				if( count($products_response['products']) == 0 ){
					$html[] = '<div class="AmazonWooCommerce-message blue">You need to import some Amazon products first!</div>';
				}
				else{
					
					$html[] = '<div class="AmazonWooCommerce-products-summary">';
					$html[] = '<div class="the-item-stat">
			                        <span style="background-color:#5A1977;" class="AmazonWooCommerce-summary-icon">
			                        	<img src="' . ( $this->module_folder . 'assets/' ) . 'total_products.png">
			                        </span>
			                        <span class="AmazonWooCommerce-summary-text">
					...
					$html[] = 	'<ul class="AmazonWooCommerce-top-products">';
					...
					if( count($products_response['products']) > 0 ){
						...
						foreach ($products_response['products'] as $product ) {
							$html[] = 		'<li>';
							$html[] = 			'<div class="AmazonWooCommerce-prod-position"><span>#' . ( ++$pos ) . '</span></div>';
							$html[] = 			'<a href="' . ( admin_url('post.php?post=' . ( $product['id'] ) . '&action=edit') ) . '" target="_blank" class="AmazonWooCommerce-the-product">';
							$html[] = 				'<div class="AmazonWooCommerce-the-product-image">' . ( get_the_post_thumbnail( $product['id'], array(75, 75) ) ) . '</div>';
		...
		private function getRemote( $the_url, $cache_lifetime=60 )
		{
			...
			$request_alias = 'AmazonWooCommerce_' . md5($the_url);
