...
if(class_exists('AmazonWooCommerce') != true) {
	class AmazonWooCommerce {
		...
		public $alias = 'AmazonWooCommerce';

		public $localizationName = 'AmazonWooCommerce';
		...
		public function assets_cron()
		{
			...
			AmazonWooCommerceAssetDownload_cronjob();
		...
		public function frontpage()
		{
			...
			if( !wp_script_is('AmazonWooCommerce-frontend') ) {
				wp_enqueue_script( 'AmazonWooCommerce-frontend' , $this->cfg['paths']['plugin_dir_url'] . '/lib/frontend/frontend.js', array( 'jquery' ) );
			}
		...
		public function amz_disclaimer_price_html( $price, $product ){
			...
			$text = '&nbsp;<em class="AmazonWooCommerce-price-info">' . sprintf( __('(as of %s)', $this->localizationName), $price_update_date) . '</em>';
		...
		public function amz_availability( $availability, $product ) {
			...
			if ( $post_id > 0 ) {
				...
				if ( !empty($meta) ) {
					...
					$availability['class'] = 'AmazonWooCommerce-availability-icon';
				}
			}
			return $availability;
		}
	...
	function get_post_id_by_meta_key_and_value($key, $value) 
	{
		...
		public function plugin_redirect() {
			if (get_option('AmazonWooCommerce_do_activation_redirect', false)) {
				...
				delete_option('AmazonWooCommerce_do_activation_redirect');
				wp_redirect( get_admin_url() . 'admin.php?page=AmazonWooCommerce' );
			}
		}
		...
		{
			add_option('AmazonWooCommerce_do_activation_redirect', true);
			add_option('AmazonWooCommerce_depedencies_is_valid', true);
			add_option('AmazonWooCommerce_depedencies_do_activation_redirect', true);
		}
		...
		public function createInstanceFreamwork ()
		{
			echo "<script type='text/javascript'>jQuery(document).ready(function ($) {
					/*var AmazonWooCommerce = new AmazonWooCommerce;
					AmazonWooCommerce.init();*/
				});</script>";
		}
		...
		public function save_options ()
		{
			...
			if( $save_id == 'AmazonWooCommerce_amazon' ){
					...
					if(class_exists('aaInterfaceTemplates') != true) {
						...
						if(trim($html) != "") {
							...
							if( isset($tryed_module[$request['section']]['in_dashboard']['icon']) ){
								$headline .= '<img src="' . ($tryed_module['folder_uri'] . $tryed_module[$request['section']]['in_dashboard']['icon'] ) . '" class="AmazonWooCommerce-headline-icon">';
							}
							$headline .= $tryed_module[$request['section']]['menu']['title'] . "<span class='AmazonWooCommerce-section-info'>" . ( $tryed_module[$request['section']]['description'] ) . "</span>";
							...
							if( $has_help === true ){
								...
								if( $help_type == 'remote' ){
									//$headline .= '<a href="#load_docs" class="AmazonWooCommerce-show-docs" data-helptype="' . ( $help_type ) . '" data-url="' . ( $tryed_module[$request['section']]['help']['url'] ) . '" data-operation="help">HELP</a>';
								} 
							}
							
							//$headline .= '<a href="#load_docs" class="AmazonWooCommerce-show-feedback" data-helptype="' . ( 'remote' ) . '" data-url="' . ( $this->feedback_url ) . '" data-operation="feedback">Feedback</a>';
		...
		*/
		...
		public function admin_load_styles()
		{
			...
			wp_enqueue_style( 'AmazonWooCommerce-aa-framework-styles', admin_url('admin-ajax.php?action=AmazonWooAdminStyles') );
		...
		*/
				...
				if( isset($module[$alias]["load_in"]['backend']) && is_array($module[$alias]["load_in"]['backend']) && count($module[$alias]["load_in"]['backend']) > 0 ){
					...
					foreach ( $module[$alias]["load_in"]['backend'] as $page ) {
						...
						if(
							...
							( ( $page == '@all' ) || ( $current_url == '/wp-admin/admin.php?page=AmazonWooCommerce' ) || ( !empty($page) && $urlfound > 0 ) )
		...
		{
			add_menu_page(
				__( 'AmazonWoo - Amazon Affiliates', $this->localizationName ),
				__( 'AmazonWoo', $this->localizationName ),
				'manage_options',
				$this->alias,
			...
			add_submenu_page(
    			$this->alias,
    			$this->alias . " " . __('Amazon Bulk Import', $this->localizationName),
	            __('Amazon Bulk Import', $this->localizationName),
	            'manage_options',
						...
						if( is_array($settings[$alias]["load_in"]['backend']) && count($settings[$alias]["load_in"]['backend']) > 0 ){
							...
							foreach ( $settings[$alias]["load_in"]['backend'] as $page ) {
								...
								if(
									// $current_url == '/wp-admin/' . $page ||
									( ( $page == '@all' ) || ( $current_url == '/wp-admin/admin.php?page=AmazonWooCommerce' ) || ( !empty($page) && $urlfound > 0 ) )
									|| ( $action == 'AmazonWooCommerceLoadSection' && $section == $alias )
									|| substr($action, 0, 3) == 'AmazonWooCommerce'
								){
									$need_to_load = true;  
								}
							}
					...
					if( $status == true && isset( $settings[$alias]['module_init'] ) ){
						if( is_file($module_folder . $settings[$alias]['module_init']) ){
							//if( is_admin() ) {
								...
								$GLOBALS['AmazonWooCommerce_current_module'] = $current_module;
		...
		public function print_plugin_usages()
		{
			...
			$html[] = '<style>
				.AmazonWooCommerce-bench-log {
					...
					.AmazonWooCommerce-bench-log th {
						font-weight: bold;
						background: #34495e;
					}
					.AmazonWooCommerce-bench-log th,
					.AmazonWooCommerce-bench-log td {
						padding: 4px 12px;
					}
				.AmazonWooCommerce-bench-title {
					position: absolute; 
					top: 55px; 
					right: 2%;
					width: 425px; 
			...
			$html[] = '<h1 class="AmazonWooCommerce-bench-title">AmazonWooCommerce: Benchmark performance</h1>';
			$html[] = '<table class="AmazonWooCommerce-bench-log">';
			$html[] = 	'<thead>';
			$html[] = 		'<tr>';
			$html[] = 			'<th>Module</th>';
		...
		*/
		...
		public function verifyProdImageHash( $hash ) {
			...
			$AmazonWooCommerceAssetDownloadCron = new AmazonWooCommerceAssetDownload();
			
			return $AmazonWooCommerceAssetDownloadCron->verifyProdImageHash( $hash );
		}
		...
		public function addNewProduct ( $retProd=array(), $import_images=true )
		{
			...
			apply_filters( 'AmazonWooCommerce_after_product_import', $lastId );
	    ...
	    */
		...
		public function download_asset_lightbox( $prod_id=0, $return='die' )
		{
			...
			$html = array();
			$html[] = '<div class="AmazonWooCommerce-asset-download-lightbox">';
			$html[] = 	'<div class="AmazonWooCommerce-donwload-in-progress-box">';
			$html[] = 		'<h1>' . __('Images download in progress ... ', $this->localizationName ) . '<a href="#" class="AmazonWooCommerce-button red" id="AmazonWooCommerce-close-btn">' . __('CLOSE', $this->localizationName ) . '</a></h1>';
			$html[] = 		'<p class="AmazonWooCommerce-message AmazonWooCommerce-info AmazonWooCommerce-donwload-notice">';
			...
			$html[] = 		'<div class="AmazonWooCommerce-process-progress-bar">';
			$html[] = 			'<div class="AmazonWooCommerce-process-progress-marker"><span>0%</span></div>';
			$html[] = 		'</div>';
			
			$html[] = 		'<div class="AmazonWooCommerce-images-tail">';
			$html[] = 			'<ul>';
			...
			$html[] = 		'
			<script>
				jQuery(".AmazonWooCommerce-images-tail ul").each(function(){
				...
				jQuery(".AmazonWooCommerce-images-tail ul").scrollLeft(0);
			</script>
			';
			
			$html[] = 		'<h2 class="AmazonWooCommerce-process-headline">' . __('Debugging Information:', $this->localizationName ) . '</h2>';
			$html[] = 		'<table class="AmazonWooCommerce-table AmazonWooCommerce-debug-info">';
			$html[] = 			'<tr>';
			...
			$html[] = 				'<td class="AmazonWooCommerce-value-downloaded">0</td>';
			$html[] = 			'</tr>';
			$html[] = 			'<tr>';
			$html[] = 				'<td>' . __('Downloaded Failures:', $this->localizationName ) . '</td>';
			$html[] = 				'<td class="AmazonWooCommerce-value-failures">0</td>';
			$html[] = 			'</tr>';
			$html[] = 		'</table>';
			
			$html[] = 		'<div class="AmazonWooCommerce-downoad-log">';
			$html[] = 			'<ol>';
		...
		public function product_assets_delete($prod_id) {
			...
			if( function_exists('get_product') ){
				...
				if ( isset($product->id) && (int) $product->id > 0 ) {
					...
					return $AmazonWooCommerceAssetDownloadCron->product_assets_delete( $prod_id );
				}  
			}
		}
		...
		public function db_custom_insert($table, $fields, $ignore=false, $wp_way=false) {
			if ( $wp_way && !$ignore ) {
				...
				$formatVals = implode(', ', array_map(array('AmazonWooCommerce', 'prepareForInList'), $fields['format']));
				$theVals = array();
		...
		public function print_module_error( $module=array(), $error_number, $title="" )
		{
			...
			$html[] = '<div class="AmazonWooCommerce-grid_4 AmazonWooCommerce-error-using-module">';
			$html[] = 	'<div class="AmazonWooCommerce-panel">';
			$html[] = 		'<div class="AmazonWooCommerce-panel-header">';
			$html[] = 			'<span class="AmazonWooCommerce-panel-title">';
			$html[] = 				__( $title, $this->localizationName );
			$html[] = 			'</span>';
			$html[] = 		'</div>';
			$html[] = 		'<div class="AmazonWooCommerce-panel-content">';
			
			$error_msg = isset($module[$module['alias']]['errors'][$error_number]) ? $module[$module['alias']]['errors'][$error_number] : '';
			
			$html[] = 			'<div class="AmazonWooCommerce-error-details">' . ( $error_msg ) . '</div>';
			$html[] = 		'</div>';
			$html[] = 	'</div>';
			$html[] = '</div>';
		...
		public function convert_to_button( $button_params=array() )
		{
			...
			$button[] = ' class="AmazonWooCommerce-button';
		...
		public function premium_message( $msg_type="row" )
		{
			...
			$html[] = '<div class="AmazonWooCommercePremiumMSG">';
			
			if( $msg_type == 'row-large' ){
				$html[] = "<h2 class='msg-title'>Like AmazonWoo Light Version? This module is available only on the premium version. </h2>";
				$html[] = "<p class='msg-intro'>Visit <a href='" . ( $this->buy_url() ) . "' target='_blank'>AmazonWoo Premium page</a> for details and full features lists.</p>";
				$html[] = "<a href='" . ( $this->buy_url() ) . "' class='AmazonWooCommerceBuyNowBtn' target='_blank'></a>";
			}
