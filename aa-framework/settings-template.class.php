		...
		public function bildThePage ( $options = array(), $alias='', $module=array(), $showForm=true ) 
		{
			...
			foreach ( $options as $theBoxs ) {
				...
				foreach ( $theBoxs as $box_id => $box ){
					...
					// container setup
					$html[] = '<div class="AmazonWooCommerce-' . ( $box['size'] ) . '">
                        	<div class="AmazonWooCommerce-' . ( $box['style'] ) . '">';
					...
					if( $box['header'] == true ) {
						$html[] = '<div class="AmazonWooCommerce-panel-header">
							<span class="AmazonWooCommerce-panel-title">
							 ...
							 ' . ( $box['toggler'] == true ? '<span class="AmazonWooCommerce-panel-toggler"></span>' : '' ) . '
						</div>';
					}
						
					$html[] = '<div class="AmazonWooCommerce-panel-content">';
					if($showForm){
						$html[] = '<form class="AmazonWooCommerce-form" id="' . ( $box_id ) . '" action="#save_with_ajax">';
					}
					...
					if(count($box['elements']) > 0){
						...
						foreach ( $box['elements'] as $elm_id => $value ){
							...
							if(!in_array( $value['type'], $noRowElements)){
								...
								$html[] = '<div class="AmazonWooCommerce-form-row' . ($this->tabsElements($box, $elm_id)) . '" style="position: relative;">';
								
								if( isset($value['premium']) && $value['premium'] == true ){
									//$html[] = $AmazonWooCommerce->premium_message('row-small');
								}
								$html[] = 	'<label for="' . ( $elm_id ) . '">' . ( isset($value['title']) ? $value['title'] : '' ) . '</label>
									   <div class="AmazonWooCommerce-form-item'. ( isset($value['size']) ? " " . $value['size'] : '' ) .'">';
							}
							...
							switch ( $value['type'] ) {
								...
								case 'upload_image':
									...
									if(!empty($val)){
										$imgSrc = $AmazonWooCommerce->image_resize( $val, $value['thumbSize']['w'], $value['thumbSize']['h'], $value['thumbSize']['zc'] );
								...
								case 'message':
									$html[] = '<div class="AmazonWooCommerce-message AmazonWooCommerce-' . ( $value['status'] ) . ' ' . ($this->tabsElements($box, $elm_id)) . '">' . ( $value['html'] ) . '</div>';
								break;
									...
									if(count($value['options']) > 0){
										foreach ($value['options'] as $key => $value){
											$html[] = '<input 
												...
												value="' . ( $value['value'] ) . '" 
												class="AmazonWooCommerce-button ' . ( $value['color'] ) . ' ' . ( isset($value['pos']) ? $value['pos'] : '' ) . ' ' . ( $value['action'] ) . '" 
											/>';
										}
									}
								...
								case 'multiselect_left2right':
									...
									$html[] = '<div class="AmazonWooCommerce-multiselect-half AmazonWooCommerce-multiselect-available" style="margin-right: 2%;">';
									...
									$html[] = '<div class="AmazonWooCommerce-multiselect-half AmazonWooCommerce-multiselect-selected">';
									...
									$html[] = '<span style="display: inline-block; width: 24.1%; text-align: center;"><input id="' . esc_attr( $elm_id ) . '-moveright" type="button" value="Move Right" class="moveright AmazonWooCommerce-button gray"></span>';
									$html[] = '<span style="display: inline-block; width: 24.1%; text-align: center;"><input id="' . esc_attr( $elm_id ) . '-moverightall" type="button" value="Move Right All" class="moverightall AmazonWooCommerce-button gray"></span>';
									$html[] = '<span style="display: inline-block; width: 24.1%; text-align: center;"><input id="' . esc_attr( $elm_id ) . '-moveleft" type="button" value="Move Left" class="moveleft AmazonWooCommerce-button gray"></span>';
									$html[] = '<span style="display: inline-block; width: 24.1%; text-align: center;"><input id="' . esc_attr( $elm_id ) . '-moveleftall" type="button" value="Move Left All" class="moveleftall AmazonWooCommerce-button gray"></span>';
									$html[] = '</div>';
								break;
							...
							if(!in_array( $value['type'], $noRowElements)){
								// close: .AmazonWooCommerce-form-row
								$html[] = '</div>';
								
								// close: .AmazonWooCommerce-form-item
								$html[] = '</div>';
							}
					...
					// AmazonWooCommerce-message use for status message, default it's hidden
					$html[] = '<div class="AmazonWooCommerce-message" id="AmazonWooCommerce-status-box" style="display:none;"></div>';
					
					if( $box['premium'] == true ){
						$html[] = $AmazonWooCommerce->premium_message('row-large');
					}
					
					if( $box['buttons'] == true && !is_array($box['buttons']) ) {
						// buttons for each box
						$html[] = '<div class="AmazonWooCommerce-button-row">
							<input type="submit" value="Save the settings" class="AmazonWooCommerce-button green AmazonWooCommerce-saveOptions" />
						</div>';
					}
					elseif( is_array($box['buttons']) ){
						...
						foreach ( $box['buttons'] as $key => $value ){
							$html[] = '<input type="submit" value="' . ( $value['value'] ) . '" class="AmazonWooCommerce-button ' . ( $value['color'] ) . ' ' . ( $value['action'] ) . '" />';
						}
					...
					// close: .AmazonWooCommerce-panel-content
					$html[] = '</div>';
					
					// close: box style  div (.AmazonWooCommerce-panel)
					$html[] = '</div>';
...
?>
		<div id="AmazonWooCommerce-wrapper" class="fluid wrapper-AmazonWooCommerce">
			...
			<?php
			// show the top menu
			AmazonWooCommerceAdminMenu::getInstance()->show_menu( $pluginPage );
			?>
				...
				<div class="AmazonWooCommerce-container clearfix">
					...
					<div id="AmazonWooCommerce-content-wrap" class="clearfix">
						...
						<div id="AmazonWooCommerce-content-area">
							<!-- Content Area -->
							<div id="AmazonWooCommerce-ajax-response"></div>
